...
class BusinessFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        spinner_country3.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
    ...
    val businessSetUpCaller = object : BusinessSetUpCaller {
        override fun setSelectedIdsList(seletedId_business: MutableList<Int>) {
            selectedIds = seletedId_business
    ...
    private val recyclerItemListener = object : RecyclerItemListener {
        override fun onLongItemSelected(position: Int): Boolean {
            ...
            activityCallback?.onBusinessSeletedItem(selectedIds, localArticles, businessSetUpCaller)
            setUpRecyclerViewWithCheckBox(localArticles)
            return true
        }

        override fun onItemSelected(position: Int) {
            activityCallback?.setBusinessCaller(businessSetUpCaller)
            if (isMultiSelect) {
                addIDIntoSelectedIds(position)
                activityCallback?.onBusinessSeletedItem(selectedIds, localArticles, businessSetUpCaller)
            } else {
    ...
    private fun setUpRecyclerView(articles: List<Articles>) {
        recycler_view_news_list3.layoutManager = LinearLayoutManager(this.context)
        recycler_view_news_list3.setHasFixedSize(true)
        recycler_view_news_list3.adapter = MainViewAdapter(articles, recyclerItemListener, false, methodCaller)
    }

    private fun setUpRecyclerViewWithCheckBox(articles: List<Articles>) {
        recycler_view_news_list3.layoutManager = LinearLayoutManager(this.context)
        recycler_view_news_list3.setHasFixedSize(true)
        recycler_view_news_list3.adapter = MainViewAdapter(articles, recyclerItemListener, true, methodCaller)
    }
